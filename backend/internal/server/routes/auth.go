...
func RegisterAuthRoutes(
	...
	auth.Use(servermiddleware.BackendModeAuthGuard(settingService))
	{
		...
		auth.POST("/billing-session/redeem", rateLimiter.LimitWithOptions("billing-session-redeem", 60, time.Minute, middleware.RateLimitOptions{
			FailureMode: middleware.RateLimitFailClose,
		}), h.Auth.RedeemBillingSession)
